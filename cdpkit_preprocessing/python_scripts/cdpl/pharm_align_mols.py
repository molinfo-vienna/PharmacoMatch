...
def parseArgs() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aligns a set of input molecules onto a given reference pharmacophore."
    )
        ...
        while mol_reader.read(mol):
            ...
            try:
                ...
                while (
                    ...
                    xform = Math.Matrix4D(
                        almnt.getTransform()
                    )  # make a copy of the alignment transformation (mol. ph4 -> ref. ph4)

                    almnt_solutions.append((score, xform))

                if not args.quiet:
                    print(
                        " -> Found %s alignment solutions" % str(len(almnt_solutions))
                    )

                saved_coords = (
                    Math.Vector3DArray()
                )  # create data structure for storing 3D coordinates

                Chem.get3DCoordinates(
                    mol, saved_coords
                )  # save the original atom coordinates

                struct_data = None

                if Chem.hasStructureData(
                    mol
                ):  # get existing structure data if available
                ...
                almnt_solutions = sorted(
                    almnt_solutions, key=lambda entry: entry[0], reverse=True
                )
                ...
                for solution in almnt_solutions:
                    ...
                    if (
                        args.min_pose_rmsd > 0.0
                        and last_pose
                        and Math.calcRMSD(last_pose, curr_pose) < args.min_pose_rmsd
                    ):
                        continue
                    ...
                    struct_data[len(struct_data) - 1].setData(
                        format(solution[0], ".4f")
                    )

                    try:
                        if not mol_writer.write(
                            mol
