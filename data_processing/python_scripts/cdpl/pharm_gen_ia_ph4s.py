...
def parseArgs() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generates pharmacophores describing the interactions between a given receptor structure and a set of ligand molecules."
    )
...
def readAndPrepareReceptorStructure(args: argparse.Namespace) -> Chem.Molecule:
    ...
    sup_fmts = [Chem.DataFormat.MOL2, Biomol.DataFormat.PDB, Biomol.DataFormat.MMTF]

    if (
        reader.getDataFormat() not in sup_fmts
    ):  # check if the format is supported by this script
        sys.exit(
            "Error: receptor input file format '%s' not supported" % name_and_ext[1]
        )
    ...
    try:
        ...
        if args.strip_res_list:
            ...
            if atoms_to_rem.numAtoms > 0:
                ...
                if not args.quiet:
                    print(
                        "! Removed %s atoms from the receptor structure"
                        % str(atoms_to_rem.numAtoms)
                    )
...
def main() -> None:
    ...
    try:
        ...
        while lig_reader.read(lig_mol):
            ...
            if not args.quiet:
                print(
                    "- Generating interaction pharmacophore of molecule %s..." % mol_id
                )
            ...
            try:
                ...
                if not args.quiet:
                    print(
                        ...
                        % (
                            str(ia_ph4.numFeatures),
                            Pharm.generateFeatureTypeHistogramString(ia_ph4),
                        )
                    )
