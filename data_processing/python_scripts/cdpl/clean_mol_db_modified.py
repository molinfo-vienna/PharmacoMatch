...
def processMolecule(
    mol: Chem.Molecule, args: argparse.Namespace
) -> tuple[Chem.MolecularGraph, str]:
    chgs_mod = False
    ...
    if args.min_charges:
        ...
        chgs_mod = Chem.ProtonationStateStandardizer().standardize(
            mol, res_mol, Chem.ProtonationStateStandardizer.MIN_CHARGED_ATOM_COUNT
        )
    ...
    if args.strip_comps:
        ...
        if comps.size > 1:  # check if multi-comp. molecule
            ...
            for comp in Chem.getComponents(mol):  # for each component
                hvy_atom_count = MolProp.getHeavyAtomCount(
                    comp
                )  # calc. non-hydrogen atom count

                if (
                    hvy_atom_count > lgst_comp_hvy_atom_count
                ):  # check if the largest comp. so far has been found
                    lgst_comp_hvy_atom_count = (
                        hvy_atom_count  # if so, store for later use
                    )
                    lgst_comp = comp
...
def checkExcludedElements(
    elem_histo: MolProp.ElementHistogram, elem_list: list
) -> bool:
    if not elem_list:
        return True
...
def parseElementList(elem_list: str) -> list[int]:
    if not elem_list:
        return []
...
def parseElementCountList(elem_count_list: str) -> dict:
    ...
    for elem_count in elem_count_list.split(","):
        ...
        if atom_type == Chem.AtomType.UNKNOWN:
            sys.exit(
                "Error: unknown chemical element/generic type '%s'" % elem_count[0]
            )
...
def main() -> None:
    ...
    # assume 'SMILES Name' format for SMILES input files
    ...
    # write 'SMILES Name' formatted lines for SMILES output
    Chem.setSMILESRecordFormatParameter(writer, "SN")
        ...
        while reader.read(in_mol):
            ...
            try:
                ...
                if not out_mol:  # check whether the molecule has been sorted out
                    ...
                    if (
                        not disc_writer
                    ):  # check whether discarded molecules should be saved to a separate file
                        i += 1
                        continue
                ...
                try:
                    ...
                    if not out_mol_writer.write(out_mol):
                        sys.exit("Error: writing molecule %s failed: %s" % (mol_id))
                ...
                sys.exit(
                    "Error: processing of molecule %s failed: %s" % (mol_id, str(e))
                )
...
def parseArgs() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Strips compounds that fulfill particular user-defined criteria from a molecule database"
    )
