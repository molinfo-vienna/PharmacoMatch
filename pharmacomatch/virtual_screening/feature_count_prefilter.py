...
class FeatureCountPrefilter:
    ...
    def get_actives_mask(self, query_idx: int = 0) -> Tensor:
        self.query_count = self.get_feature_count(self.vs_datamodule.query_dataloader())
        self.actives_count = self.get_feature_count(
            self.vs_datamodule.actives_dataloader()
        )
        actives_count = self.actives_count - self.query_count[query_idx].reshape(1, -1)
    ...
    def get_inactives_mask(self, query_idx: int = 0) -> Tensor:
        ...
        inactives_count = self.inactives_count - self.query_count[query_idx].reshape(
            1, -1
        )
