...
class VirtualScreening:
    ...
    def perform_screening(self, datamodule):
        ...
        self.plot_UMAP(
            query, actives, inactives, active_similarity, inactive_similarity
        )
    ...
    def plot_UMAP(
        self, query, actives, inactives, active_similarity, inactive_similarity
    ):
        labels = torch.cat(
            (
        ...
        similarity = torch.cat(
            (
                inactive_similarity,
                active_similarity,
                torch.tensor([1], device=active_similarity.device),
            )
        )
        reducer = umap.UMAP()
